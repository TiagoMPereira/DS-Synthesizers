...
def plot_histogram(original: pd.DataFrame, synthetic: pd.DataFrame, save_path: str = None):
    ...
    fig, ax = plt.subplots(nrows=nrows, ncols=ncols, figsize=(ncols*4,nrows*4), sharey='row', sharex='row')
...
def plot_kde(original: pd.DataFrame, synthetic: pd.DataFrame, save_path: str = None):
    columns = list(original.columns)
    assert list(synthetic.columns) == columns
...
def plot_pca(original: pd.DataFrame, synthetic: pd.DataFrame, save_path: str = None):
    ...
    plt.scatter(pca_generated[:, 0], pca_generated[:, 1], label="synthetic data")
    plt.legend()
